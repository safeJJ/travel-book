...
const defaultSettings: TripSettings = { name: "แก๊งหลงทางเชียงใหม่ ✈️", destination: "เชียงใหม่", leader: "SafeJJ", inviteEnabled: true, inviteCode: "CHIANG-MAI" };
...
const defaultMemories: Memory[] = [{ id: 1, emoji: "🏔️", caption: "วิวแรกของทริป" }, { id: 2, emoji: "☕", caption: "กาแฟแก้วแรก" }];
const defaultPlaces: Place[] = [{ id: 1, emoji: "🧱", name: "ประตูท่าแพ" }, { id: 2, emoji: "🏡", name: "บ้านพักริมปิง" }, { id: 3, emoji: "🍜", name: "ข้าวซอยแม่สาย" }];
const defaultMembers: Member[] = [{ id: 1, avatar: "👑", name: "SafeJJ", role: "หัวหน้าทริป" }, { id: 2, avatar: "😎", name: "เมย์", role: "สมาชิก" }, { id: 3, avatar: "🐻", name: "ปอนด์", role: "สมาชิก" }];
...
function ask(label: string, currentValue: string) {
  return window.prompt(label, currentValue)?.trim() || null;
}
...
export default function SimpleTripPlatform() {
  ...
  const nextAction = useMemo(() => {
    ...
    return { icon: "🗓️", title: "ดูแผนวันถัดไป", detail: `${currentActivities.length} กิจกรรมในวันที่ ${activeDay}`, action: "plan" as const };
  }, [dates, places.length, currentActivities.length, activeDay]);

  function toggleEditMode() {
    const next = !editMode;
    setEditMode(next);
    writeStorage(STORAGE_KEYS.editMode, next);
  ...
  function saveSettings(event: FormEvent) {
    ...
    persistMembers(members.map((member) => member.role === "หัวหน้าทริป" ? { ...member, name: settings.leader } : member));
  ...
  function editMemory(memory: Memory) { const emoji = ask("แก้ไขอีโมจิ", memory.emoji); if (!emoji) return; const caption = ask("แก้ไขข้อความ", memory.caption); if (caption) persistMemories(memories.map((item) => item.id === memory.id ? { ...item, emoji, caption } : item)); }
  ...
  function editPlace(place: Place) { const emoji = ask("แก้ไขอีโมจิ", place.emoji); if (!emoji) return; const name = ask("แก้ไขชื่อสถานที่", place.name); if (name) persistPlaces(places.map((item) => item.id === place.id ? { ...item, emoji, name } : item)); }
  function deletePlace(id: number) { if (window.confirm("ลบสถานที่นี้หรือไม่?")) persistPlaces(places.filter((place) => place.id !== id)); }
  function addMember() { const name = ask("ชื่อสมาชิกใหม่", "เพื่อนใหม่"); if (!name) return; const avatar = ask("อีโมจิประจำตัว", "🙂") || "🙂"; persistMembers([...members, { id: Date.now(), avatar, name, role: "สมาชิก" }]); }
  function editMember(member: Member) { const name = ask("แก้ไขชื่อสมาชิก", member.name); if (!name) return; const avatar = ask("แก้ไขอีโมจิ", member.avatar) || member.avatar; const role = ask("แก้ไขบทบาท", member.role) || member.role; persistMembers(members.map((item) => item.id === member.id ? { ...item, name, avatar, role } : item)); }
  ...
  const controls = (onEdit: () => void, onDelete: () => void) => editMode ? <div className={styles.rowActions}><button type="button" onClick={onEdit}>แก้</button><button type="button" onClick={onDelete}>ลบ</button></div> : null;
    ...
    <main className={styles.shell}>
      <section className={styles.phone}>
        <header className={styles.hero}>
          <div className={styles.heroTop}>
            ...
            <div className={styles.modeControls}>
              <button className={editMode ? styles.editModeActive : ""} type="button" onClick={toggleEditMode}>{editMode ? "✓ แก้ไข" : "✎ แก้ไข"}</button>
              {editMode && <button type="button" onClick={() => setShowSettings(true)} aria-label="ตั้งค่าทริป">•••</button>}
            </div>
          </div>
          <h1>{settings.name}</h1><p>{dates?.label ?? "ยังไม่ได้เลือกวันเดินทาง"}</p>
          ...
          <div className={styles.modeStatus}>{editMode ? "โหมดแก้ไข · เพิ่ม แก้ และลบข้อมูลได้" : "โหมดปกติ · ดูและใช้งานทริป"}</div>
        </header>
        ...
        {tab === "home" && <div className={styles.content}>
          ...
          <section className={styles.simpleSection}><div className={styles.sectionTitle}><h2>ภาพรวม</h2></div><div className={styles.summaryGrid}><button type="button" onClick={() => setTab("plan")}><span>🗓️</span><strong>{Object.values(plan).flat().length}</strong><small>กิจกรรม</small></button><button type="button" onClick={() => setTab("tasks")}><span>✅</span><strong>{completedTasks}/{tasks.length}</strong><small>งานเสร็จแล้ว</small></button><button type="button" onClick={() => setTab("plan")}><span>📍</span><strong>{places.length}</strong><small>สถานที่</small></button><button type="button" onClick={() => setTab("memories")}><span>📸</span><strong>{memories.length}</strong><small>ความทรงจำ</small></button></div></section>
          <section className={styles.simpleSection}><div className={styles.sectionTitle}><h2>เพื่อนร่วมทริป</h2><div className={styles.titleActions}><button type="button" onClick={copyInvite}>{copied ? "คัดลอกแล้ว" : "ชวนเพื่อน"}</button>{editMode && <button type="button" onClick={addMember}>+ เพิ่ม</button>}</div></div>{members.map((member) => <div className={styles.memberRow} key={member.id}><span>{member.avatar}</span><div><strong>{member.name}</strong><small>{member.role}</small></div>{controls(() => editMember(member), () => deleteMember(member))}</div>)}</section>
        </div>}

        {tab === "plan" && <div className={styles.content}>
          <div className={styles.pageTitle}><div><small>แผนทริป</small><h2>{settings.destination}</h2></div>{editMode ? <Link href="/date-vote">แก้วัน</Link> : <span>{dates?.label ?? "ยังไม่กำหนดวัน"}</span>}</div>
          ...
          <div className={styles.timeline}>{currentActivities.length === 0 ? <div className={styles.empty}>วันนี้ยังไม่มีแผน</div> : currentActivities.slice().sort((a, b) => a.time.localeCompare(b.time)).map((activity) => <article key={activity.id}><time>{activity.time}</time><div><strong>{activity.title}</strong><small>{activity.note}</small></div>{controls(() => editActivity(activity), () => deleteActivity(activity.id))}</article>)}</div>
          {editMode && <form className={styles.inlineForm} onSubmit={addActivity}><input type="time" value={activityTime} onChange={(event) => setActivityTime(event.target.value)} /><input placeholder="เพิ่มกิจกรรม" value={activityTitle} onChange={(event) => setActivityTitle(event.target.value)} /><button type="submit">เพิ่ม</button></form>}
          <section className={styles.simpleSection}><div className={styles.sectionTitle}><h2>สถานที่ที่เก็บไว้</h2></div><div className={styles.placeList}>{places.map((place) => <div key={place.id}><span>{place.emoji}</span><strong>{place.name}</strong>{controls(() => editPlace(place), () => deletePlace(place.id))}</div>)}</div>{editMode && <form className={styles.inlineForm} onSubmit={addPlace}><input className={styles.fullInput} placeholder="เพิ่มสถานที่" value={placeName} onChange={(event) => setPlaceName(event.target.value)} /><button type="submit">เพิ่ม</button></form>}</section>
        </div>}

        {tab === "tasks" && <div className={styles.content}>
          <div className={styles.pageTitle}><div><small>งานที่ต้องทำ</small><h2>{completedTasks}/{tasks.length} งานเสร็จแล้ว</h2></div></div>
          <div className={styles.taskList}>{tasks.map((task) => <div className={styles.taskRow} key={task.id}><button className={styles.taskToggle} type="button" onClick={() => toggleTask(task.id)}><span className={task.done ? styles.doneCheck : styles.check}>{task.done ? "✓" : ""}</span><strong className={task.done ? styles.doneText : ""}>{task.title}</strong></button>{controls(() => editTask(task), () => deleteTask(task.id))}</div>)}</div>
          {editMode && <form className={styles.inlineForm} onSubmit={addTask}><input className={styles.fullInput} placeholder="เพิ่มงานใหม่" value={taskTitle} onChange={(event) => setTaskTitle(event.target.value)} /><button type="submit">เพิ่ม</button></form>}
        </div>}

        {tab === "memories" && <div className={styles.content}>
          <div className={styles.pageTitle}><div><small>สมุดทริป</small><h2>เรื่องราวของพวกเรา</h2></div></div>
          <div className={styles.memoryGrid}>{memories.map((memory) => <article key={memory.id}><div>{memory.emoji}</div><strong>{memory.caption}</strong>{editMode && <div className={styles.cardActions}><button type="button" onClick={() => editMemory(memory)}>แก้ไข</button><button type="button" onClick={() => deleteMemory(memory.id)}>ลบ</button></div>}</article>)}</div>
          {editMode && <form className={styles.inlineForm} onSubmit={addMemory}><input className={styles.fullInput} placeholder="วันนี้มีอะไรน่าจำ?" value={memoryCaption} onChange={(event) => setMemoryCaption(event.target.value)} /><button type="submit">เพิ่ม</button></form>}
        </div>}

        <nav className={styles.nav} aria-label="เมนูหลัก"><button className={tab === "home" ? styles.activeNav : ""} type="button" onClick={() => setTab("home")}><span>⌂</span><small>หน้าหลัก</small></button><button className={tab === "plan" ? styles.activeNav : ""} type="button" onClick={() => setTab("plan")}><span>▤</span><small>แผนทริป</small></button><button className={tab === "tasks" ? styles.activeNav : ""} type="button" onClick={() => setTab("tasks")}><span>✓</span><small>งาน</small></button><button className={tab === "memories" ? styles.activeNav : ""} type="button" onClick={() => setTab("memories")}><span>▧</span><small>สมุดทริป</small></button></nav>

        {showSettings && editMode && <div className={styles.overlay} role="dialog" aria-modal="true" aria-label="ตั้งค่าทริป"><form className={styles.sheet} onSubmit={saveSettings}><div className={styles.sheetTitle}><h2>ตั้งค่าทริป</h2><button type="button" onClick={() => setShowSettings(false)}>×</button></div><label>ชื่อทริป<input value={settings.name} onChange={(event) => setSettings({ ...settings, name: event.target.value })} /></label><label>จุดหมาย<input value={settings.destination} onChange={(event) => setSettings({ ...settings, destination: event.target.value })} /></label><label>หัวหน้าทริป<input value={settings.leader} onChange={(event) => setSettings({ ...settings, leader: event.target.value })} /></label><label>รหัสลิงก์เชิญ<input value={settings.inviteCode} onChange={(event) => setSettings({ ...settings, inviteCode: event.target.value })} /></label><label className={styles.switchRow}><span>เปิดลิงก์เชิญ</span><input type="checkbox" checked={settings.inviteEnabled} onChange={(event) => setSettings({ ...settings, inviteEnabled: event.target.checked })} /></label><button className={styles.primary} type="submit">บันทึก</button></form></div>}
      </section>
      <aside className={styles.guide}><span>TWO MODES</span><h2>เรียบง่ายตอนใช้ ชัดเจนตอนแก้</h2><p>โหมดปกติซ่อนเครื่องมือจัดการทั้งหมด ส่วนโหมดแก้ไขจะแสดงปุ่มเพิ่ม แก้ และลบเฉพาะเมื่อจำเป็น</p><ol><li>สลับโหมดจากด้านบน</li><li>ระบบจำโหมดล่าสุด</li><li>งานยังติ๊กเสร็จได้ในโหมดปกติ</li><li>ข้อมูลทั้งหมดเก็บใน Mockup เดิม</li></ol></aside>
    </main>
  );
}
